...
def proxy_image():
    ...
    if topic:
        try:
            wiki_url = f"https://en.wikipedia.org/w/api.php?action=query&titles={urllib.parse.quote(topic)}&prop=pageimages&format=json&pithumbsize=800&redirects=1"
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                ...
                for page_id in pages:
                    if 'thumbnail' in pages[page_id]:
                        ...
                        with urllib.request.urlopen(img_req, timeout=10) as final_res:
                            return Response(final_res.read(), mimetype=final_res.headers.get('Content-Type', 'image/jpeg'))
        except Exception as e:
            print(f"Wikipedia fallback failed: {e}")

    return jsonify({"error": "Failed to fetch any matching image"}), 500
